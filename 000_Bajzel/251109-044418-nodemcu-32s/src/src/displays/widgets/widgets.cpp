...
void VuWidget::_draw(){
  ...
  uint16_t bandColor;
  uint16_t dimension = _bands.width;
  ...
  //-------> Adăugat: Cadre de desen și litere L/R cu centrare și cadre mai mari
  const int labelHeight = 15;                       // Înălțimea chenarului a fost mărită cu 2 pixeli (20 pixeli + 2 pixeli = 22 pixeli)
  const int labelL = _config.top ;  // Coborârea marginilor cu 6 pixeli
  const int labelR = _config.top + 15;  // Coborârea marginilor cu 6 pixeli
  const int labelTextSize = 1;
  const int labelFontWidth = 5 * labelTextSize;   // Lățimea textului 'L' i 'R'
  const int labelFontHeight = 7 * labelTextSize;  // Înălțimea textului 'L' i 'R'

  // Ramka L
  dsp.fillRect(8, labelL - 2, 10, labelHeight, 0xFFFF );
  // Ramka R
  dsp.fillRect(8, labelR + 10, 10, labelHeight, 0xFFFF );
//dsp.fillRect(_config.left + _dimension+10, _config.top, _dimension+10, (_dimension / 2+3) , _fgcolor); //PTR CASETA

  dsp.setTextColor( 0x0000, 0xFFFF );
  dsp.setTextSize(labelTextSize);

// CHENAR
//dsp.drawRect(35, labelR - 19, 150, 50, _vumaxcolor);

  // Centrat litera L
  dsp.setCursor(
     10,  // Centrat pe orizontală într-un cadru în formă de L
    labelL + (labelHeight - labelFontHeight + 3 ) / 2 - 3      // Centrați vertical în cadrul L
  );
  dsp.print("L");

  // Centrat litera R
  dsp.setCursor(
     10 ,  // Centrare orizontală în cadrul R
    labelR + (labelHeight + 20 ) / 2 - 3                                   // Centrare verticală în cadrul R
  );
  dsp.print("R");
  //-------- <<<
  ...
  for(int i=0; i<dimension; i++){
    if(i%(dimension/_bands.perheight)==0){
      bandColor = (i>_bands.width-(_bands.width/_bands.perheight)*5)?_vumaxcolor:_vumincolor;
...
void VuWidget::_clear(){
  dsp.fillRect(_config.left, _config.top, _bands.width  + _bands.space, _bands.height*2+_bands.space, _bgcolor);
}
#else // DSP_LCD
VuWidget::~VuWidget() { }
void VuWidget::init(WidgetConfig wconf, VUBandsConfig bands, uint16_t vumaxcolor, uint16_t vumincolor, uint16_t bgcolor) {
...
//  if(!_active || _format == BF_UNCNOWN || _bitrate==0) return;
  ...
  dsp.drawRect(_config.left, _config.top, _dimension * 2+20, (_dimension / 2) + 3, _fgcolor); //PTR CASETA
    dsp.fillRect(_config.left + _dimension+10, _config.top, _dimension+10, (_dimension / 2+3) , _fgcolor); //PTR CASETA
...
//  dsp.setCursor(_config.left + _dimension/2 - _charWidth*strlen(_buf)/2 + 1, _config.top + _dimension/4 - _textheight/2+1);
 // dsp.setCursor(_config.left + _dimension/2 - _charWidth*strlen(_buf)/2 + 10, _config.top + _dimension/4 - _textheight/2 + 22);
 //dsp.setCursor(_config.left + _dimension/2 - _charWidth*strlen(_buf)/2 + 1, _config.top + _dimension/3 - _textheight/2 + 2);
  dsp.setCursor(_config.left + _dimension / 2 - _charWidth * strlen(_buf) / 2+6, _config.top + _dimension / 4 - _textheight / 2+3);

  dsp.print(_buf);
  dsp.setTextColor(_bgcolor, _fgcolor);
  dsp.setCursor(_config.left + _dimension+5 + _dimension / 2 - _charWidth * 3 / 2+10, _config.top + _dimension / 4 - _textheight / 2+3 );

  //dsp.setCursor(_config.left + _dimension/2 - _charWidth*3/2 + 1, _config.top + _dimension - _dimension/4 - _textheight/2);
  //dsp.setCursor(_config.left + _dimension/2 - _charWidth*3/2 + 1, _config.top + _dimension - _dimension/5- _textheight/2);
...
void BitrateWidget::_clear() {
  dsp.fillRect(_config.left, _config.top, _dimension * 2, _dimension / 2, _bgcolor);
    ...
    dsp.fillRect(_config.left, _config.top, _dimension * 2+20, _dimension, _bgcolor);
}
