...
//WybÛr ekranu nr i struktura go przechowujπca
void PCD8544_NR(uint8_t NR, TPCD8544 *LCD )
{
PCD8544_SCE.WIDTH=LCD[NR].WIDTH;
PCD8544_SCE.HEIGHT=LCD[NR].HEIGHT;
PCD8544_SCE.SCEN=LCD[NR].SCEN;
//_PCD8544_W8BIT(PCD8544_COM, 0x21 );  // LCD Extended Commands.
//_PCD8544_W8BIT(PCD8544_COM, 0xFF );  // Set LCD Vop (Contrast). //B1
//_PCD8544_W8BIT(PCD8544_COM, 0x06 );  // Set Temp coefficent. //0x04
//_PCD8544_W8BIT(PCD8544_COM, 0x13 );  // LCD bias mode 1:48. //0x13
_PCD8544_W8BIT(PCD8544_COM, 0x20 );
_PCD8544_W8BIT(PCD8544_COM, 0x0C );  // LCD in normal mode. 0x0d for inverse
};


void _PCD8544_W8BIT(uint8_t DC, uint8_t OCTET)
{
	if(DC) HIGH(PCD8544_DC); else LOW(PCD8544_DC);

    uint8_t m;
    LOW(PCD8544_SCE.SCEN);
     for(m=0x80; m; m>>=1)
    {
       if(OCTET & m)
          HIGH(PCD8544_SDIN);
 ...
 //      _delay_ms(1);
     ...
     HIGH(PCD8544_SCE.SCEN);
}

void PCD8544_INIT(void)
{
  OUTPUT(PCD8544_SCE.SCEN);
  OUTPUT(PCD8544_RES);
  OUTPUT(PCD8544_DC);
  OUTPUT(PCD8544_SDIN);
  OUTPUT(PCD8544_SCLK);
...
//  _PCD8544_W8BIT(PCD8544_COM, PCD8544_FUNCTIONSET | PCD8544_EXTENDEDINSTRUCTION );  // LCD Extended Commands.
//    _PCD8544_W8BIT(PCD8544_COM, PCD8544_SETVOP | 0x7f );  // Set LCD Vop (Contrast). //B1
//    _PCD8544_W8BIT(PCD8544_COM, 0x06 );  // Set Temp coefficent. //0x04
//    _PCD8544_W8BIT(PCD8544_COM, PCD8544_SETBIAS | 4 );  // LCD bias mode 1:48. //0x13 4 optimal
//    _PCD8544_W8BIT(PCD8544_COM, PCD8544_FUNCTIONSET );  // LCD in normal mode. 0x0d for inverse
//    _PCD8544_W8BIT(PCD8544_COM, PCD8544_DISPLAYCONTROL | PCD8544_DISPLAYNORMAL );
//


//  _PCD8544_W8BIT(PCD8544_COM, 0x21 );  // LCD Extended Commands.
//  _PCD8544_W8BIT(PCD8544_COM, 0xBF );  // Set LCD Vop (Contrast). //B1
//  _PCD8544_W8BIT(PCD8544_COM, 0x04 );  // Set Temp coefficent. //0x04
//  _PCD8544_W8BIT(PCD8544_COM, 0x14 );  // LCD bias mode 1:48. //0x13
//  _PCD8544_W8BIT(PCD8544_COM, 0x20 );
//  _PCD8544_W8BIT(PCD8544_COM, 0x0C );  // LCD in normal mode. 0x0d for inverse


   _PCD8544_W8BIT(PCD8544_COM, 0x21 );  // LCD Extended Commands.
   _PCD8544_W8BIT(PCD8544_COM, 0xC8 );  // Set LCD Vop (Contrast). //B1
   _PCD8544_W8BIT(PCD8544_COM, 0x04 );  // Set Temp coefficent. //0x04
   _PCD8544_W8BIT(PCD8544_COM, 0x14 );  // LCD bias mode 1:48. //0x13
   _PCD8544_W8BIT(PCD8544_COM, 0x20 );  // LCD Standard Commands,Horizontal addressing mode
   _PCD8544_W8BIT(PCD8544_COM, 0x0C );  // LCD in normal mode. 0x0d for inverse
...
{
	for(uint8_t nrLcd=0;nrLcd<SIZE;nrLcd++) //ustawiamy nÛønki AVR na wyjúcia
		{
			PCD8544_NR(nrLcd,LCD);
			PCD8544_INIT();
			PCD8544_HOME();
//			PCD8544_TEXT("EKRAN nR",bold);
...
{
    _PCD8544_W8BIT(PCD8544_COM, 0x21);

    // Ustawienie poziomu kontrastu 0x00 do 0x7F
    _PCD8544_W8BIT(PCD8544_COM, 0x80 | CONTRAST );
...
void PCD8544_MODE(uint8_t MODE){
	switch (MODE){
		case normal: _PCD8544_W8BIT(PCD8544_COM,0x0C);
		break;
		case invert: _PCD8544_W8BIT(PCD8544_COM,0x0D);
		break;
		case blank: _PCD8544_W8BIT(PCD8544_COM,0x08);
		break;
		case allon: _PCD8544_W8BIT(PCD8544_COM,0x09);
	    break;
	}
}
...
{
    uint16_t index;
	for (index = 0; index < PCD8544_SCE.WIDTH * PCD8544_SCE.HEIGHT / 8; index++)
  {
    _PCD8544_W8BIT(PCD8544_DAT, 0x00);
   // _delay_ms(100);
  }
}
...
{
  _PCD8544_W8BIT(PCD8544_COM, 0x80 | PX);  // Column.
  _PCD8544_W8BIT(PCD8544_COM, 0x40 | PY8);  // Row.
...
//  uint8_t rev=((EFFECT&revers)/revers);
  ...
  while (spc--){
	  ...
	  _PCD8544_W8BIT(PCD8544_DAT, new | (((EFFECT&bold)/bold) * old)) ;
       old=new;
  }
...
void PCD8544_GFX2LCD(uint8_t MEMTYPE,unsigned char IMG[],int8_t POSX,int8_t POSY8,int8_t WIDTH,int8_t HEIGHT8){
	...
	for(x=0,y=0,index=0; y<HEIGHT8; y++)
	   {
		_PCD8544_W8BIT(PCD8544_COM, 0x40|(POSY8+y)); // Ustawiamy wspÛ≥rzÍdnπ Y obszaru kopiowania
		_PCD8544_W8BIT(PCD8544_COM, 0x80|(POSX)); // a teraz wspÛ≥rzÍdnπ X
		for(x=0; x<WIDTH ; x++,index++){
		    if (MEMTYPE)
			_PCD8544_W8BIT(PCD8544_DAT, pgm_read_byte(&IMG[index]));
		    else
		    _PCD8544_W8BIT(PCD8544_DAT, IMG[index]);
		}
	   }
}
