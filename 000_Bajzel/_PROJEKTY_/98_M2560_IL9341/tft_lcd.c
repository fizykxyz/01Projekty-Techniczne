...
//	uint8_t i = 0;
	...
	/*rotation = 0;
	 ...
	 textcolor = 0xFFFF;
	 LCD_WIDTH = TFTWIDTH;
	 LCD_HEIGHT = TFTHEIGHT;*/

	//CD = RS PIN
	LCD_DDR |= (1 << LCD_CS_PIN) | (1 << LCD_CD_PIN) | (1 << LCD_WR_PIN)
			| (1 << LCD_RD_PIN);

	LCD_PORT |= (1 << LCD_CS_PIN) | (1 << LCD_CD_PIN) | (1 << LCD_WR_PIN)
			| (1 << LCD_RD_PIN);
	...
	/*
	 ...
	 a = pgm_read_word(&ILI932x_regValues[i++]);d = pgm_read_word(&ILI932x_regValues[i++]);if(a == TFTLCD_DELAY) _delay_ms(d);
...
//	return;
	/*
	 ...
	 &HX8357D_regValues[i++]);uint8_t len = pgm_read_byte(&HX8357D_regValues[i++]);if(r == TFTLCD_DELAY) {
	 _delay_ms(len);
	 } else {
	 ...
	 uint8_t x = pgm_read_byte(
	 &HX8357D_regValues[i++]);write8(x);
	 }
	 CS_IDLE;
	 ...
	 a = pgm_read_byte(&HX8347G_regValues[i++]);d = pgm_read_byte(
	 &HX8347G_regValues[i++]);
	 if(a == TFTLCD_DELAY) _delay_ms(d);
...
/*
 static const uint8_t HX8347G_regValues[] PROGMEM = { 0x2E, 0x89, 0x29, 0x8F,
 0x2B, 0x02, 0xE2, 0x00, 0xE4, 0x01, 0xE5, 0x10, 0xE6, 0x01, 0xE7, 0x10,
 0xE8, 0x70, 0xF2, 0x00, 0xEA, 0x00, 0xEB, 0x20, 0xEC, 0x3C, 0xED, 0xC8,
 0xE9, 0x38, 0xF1, 0x01,
...
void setLR(void) {
	CS_ACTIVE
	;
	writeRegisterPair(HX8347G_COLADDREND_HI, HX8347G_COLADDREND_LO,
	LCD_WIDTH - 1);
...
void tft_drawFastHLine(int16_t x, int16_t y, int16_t length, uint16_t color) {
	int16_t x2;

	// Initial off-screen clipping
	if ((length <= 0) || (y < 0) || (y >= LCD_HEIGHT) || (x >= LCD_WIDTH)
			|| ((x2 = (x + length - 1)) < 0))
		return;
...
void tft_drawFastVLine(int16_t x, int16_t y, int16_t length, uint16_t color) {
	int16_t y2;

	// Initial off-screen clipping
	if ((length <= 0) || (x < 0) || (x >= LCD_WIDTH) || (y >= LCD_HEIGHT)
			|| ((y2 = (y + length - 1)) < 0))
		return;
	if (y < 0) {         // Clip top
		length += y;
...
void tft_fillRect(int16_t x1, int16_t y1, int16_t w, int16_t h,
	...
	// Initial off-screen clipping
	if ((w <= 0) || (h <= 0) || (x1 >= LCD_WIDTH) || (y1 >= LCD_HEIGHT)
...
void tft_setRotation(uint8_t x) {
	...
	if (driver == ID_932X) {
		...
		setAddrWindow(0, 0, LCD_WIDTH - 1, LCD_HEIGHT - 1); // CS_IDLE happens here
