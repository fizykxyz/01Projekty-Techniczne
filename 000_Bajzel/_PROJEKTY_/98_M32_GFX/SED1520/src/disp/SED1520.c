...
#define SET_PTR_COL     0x00

#define DISP_NORM_ORD   0xA0
#define DISP_INV_ORD    0xA1

#define SET_STATIC      0xA5
#define RST_STATIC      0xA4

#define SET_DUTY16      0xA8
#define SET_DUTY32      0xA9
...
uint8_t Disp_init(void) {
  ...
  for (uint8_t i=0; i<DISP_CHIP_NUM; i++) {
    ...
    write_buffer(DISP_INVERSED?
                 DISP_INV_ORD : DISP_NORM_ORD, COMMAND | i);
    write_buffer(RST_STATIC, COMMAND | i);
    write_buffer((DISP_DUTY_CYCLE == 16)?
                 SET_DUTY16 : SET_DUTY32, COMMAND | i);
  }
...
void Disp_scroll_window(uint8_t dir) {
  ...
  for (uint8_t i=0; i<DISP_CHIP_NUM; i++)
    write_buffer(SET_PTR_OFFSET | (shift & 0x1F), COMMAND | i);
}
...
void Disp_set_pxy(uint8_t px, uint8_t py) {
  ...
  uint8_t new_virt_x;
    ...
    active_module = DISP_SCR_X_NUM * (py / 32);
    new_virt_x = px;
    ...
    if (px < DISP_G1_SEG_NUM) {
      ...
      active_module = DISP_SCR_X_NUM * (py / 32) + 1;
      new_virt_x = px - (DISP_G1_SEG_NUM);
    }
    
  #elif DISP_SCR_X_NUM == 3
  
    if (px < DISP_G1_SEG_NUM) {
      active_module = DISP_SCR_X_NUM * (py / 32);
      new_virt_x = px;
    } else if (px < DISP_G1_SEG_NUM + DISP_G2_SEG_NUM) {
      active_module = DISP_SCR_X_NUM * (py / 32) + 1;
      new_virt_x = px - (DISP_G1_SEG_NUM);
    } else {
      active_module = DISP_SCR_X_NUM * (py / 32) + 2;
      new_virt_x = px - (DISP_G1_SEG_NUM + DISP_G2_SEG_NUM);
    }
    
  #elif DISP_SCR_X_NUM == 4
    ...
    if (px < DISP_G1_SEG_NUM) {
      ...
      active_module = DISP_SCR_X_NUM * (py / 32) + 1;
      new_virt_x = px - (DISP_G1_SEG_NUM);
    } else if (px < DISP_G1_SEG_NUM + DISP_G2_SEG_NUM + DISP_G3_SEG_NUM) {
      active_module = DISP_SCR_X_NUM * (py / 32) + 2;
      new_virt_x = px - (DISP_G1_SEG_NUM + DISP_G2_SEG_NUM);
    } else {
      active_module = DISP_SCR_X_NUM * (py / 32) + 3;
      new_virt_x = px - (DISP_G1_SEG_NUM + DISP_G2_SEG_NUM + DISP_G3_SEG_NUM);
    }
    
  #endif
  
  uint8_t new_virt_y = (py / 8) % 4;
